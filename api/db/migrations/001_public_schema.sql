...
CREATE TABLE IF NOT EXISTS tenants (
  ...
  outstanding_method  VARCHAR(20)  NOT NULL DEFAULT 'direct_upload'
                        CHECK (outstanding_method IN ('direct_upload','computed')),
  woi_red_threshold   NUMERIC(5,2) DEFAULT 4.0,
  woi_amber_threshold NUMERIC(5,2) DEFAULT 8.0,
...
-- Migration 004: outstanding method + WOI thresholds
ALTER TABLE tenants ADD COLUMN IF NOT EXISTS outstanding_method VARCHAR(20) NOT NULL DEFAULT 'direct_upload'
  CHECK (outstanding_method IN ('direct_upload','computed'));
ALTER TABLE tenants ADD COLUMN IF NOT EXISTS woi_red_threshold   NUMERIC(5,2) DEFAULT 4.0;
ALTER TABLE tenants ADD COLUMN IF NOT EXISTS woi_amber_threshold NUMERIC(5,2) DEFAULT 8.0;

-- Migration 005: target_woi_weeks + Busy Web Service API config columns
ALTER TABLE tenants ADD COLUMN IF NOT EXISTS target_woi_weeks    NUMERIC(5,2)  DEFAULT 12.0;
ALTER TABLE tenants ADD COLUMN IF NOT EXISTS busy_host           VARCHAR(200);
ALTER TABLE tenants ADD COLUMN IF NOT EXISTS busy_port           INT           DEFAULT 981;
ALTER TABLE tenants ADD COLUMN IF NOT EXISTS busy_username       VARCHAR(100);
ALTER TABLE tenants ADD COLUMN IF NOT EXISTS busy_password_enc   TEXT;
ALTER TABLE tenants ADD COLUMN IF NOT EXISTS busy_enabled        BOOLEAN       DEFAULT FALSE;
ALTER TABLE tenants ADD COLUMN IF NOT EXISTS busy_last_sync_at   TIMESTAMPTZ;
